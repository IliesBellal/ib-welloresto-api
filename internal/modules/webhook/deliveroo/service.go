...
func (s *DeliverooService) buildCustomerRequest(merchantID string, ord DeliverooOrder) (*models.CustomerRequest, error) {
	var name, phone, code, address, latStr, lngStr string
	var lat, lng float64
	...
	if isRestaurantFulfillment && ord.Delivery != nil {
		...
		code = ord.Delivery.ContactAccessCode
		...
		code = ord.Customer.ContactAccessCode
...
func (s *DeliverooService) ProcessStatusUpdate(ctx context.Context, payload DeliverooPayload) error {
	...
	case "accepted":
		// Logique de date
		// $date_now + 30 min (SCHEDULED_MIN_DELAY)
		// $dateRecueObj > $dateActuelle

		scheduledMinDelay := 30 // minutes
		now := time.Now()
		// attention: PHP fait un modify +30 sur date_now mais ne l'utilise pas vraiment dans la condition if,
		// il utilise $dateRecueObj > $dateActuelle (sans le +30).
		// Re-lecture du code PHP :
		// $date_now->modify('+30'); ... $dateRecueObj > $dateActuelle (où $dateActuelle = new DateTime())
		// Donc la condition est simple: PrepareFor > Now.
