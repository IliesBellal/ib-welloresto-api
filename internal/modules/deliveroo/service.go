...
const defaultDeliverooMenuID = "main"
...
// GetMenu récupère le menu du restaurant depuis l'API Deliveroo. locationID
// vide cible le compte "principal" du marchand (comportement historique,
// identique pour un marchand mono-compte) ; locationID renseigné cible ce
// compte précis.
func (s *DeliverooService) GetMenu(ctx context.Context, merchantID, locationID string) (map[string]interface{}, error) {
	account, err := s.repo.resolveAccount(ctx, merchantID, locationID)
	if err != nil {
		return nil, err
	}
	return s.client.GetMenu(ctx, account.BrandID)
}

// SyncMenu pousse un menu interne vers l'API Deliveroo. locationID vide cible
// le compte "principal" du marchand (comportement historique, identique pour
// un marchand mono-compte) ; locationID renseigné cible ce compte précis -
// c'est ce qui permet de choisir quel compte Deliveroo synchroniser.
func (s *DeliverooService) SyncMenu(ctx context.Context, merchantID, locationID string, menu interface{}) error {
	account, err := s.repo.resolveAccount(ctx, merchantID, locationID)
	if err != nil {
		return err
	}
	return s.client.SyncMenu(ctx, account.BrandID, menu)
}

// ToggleItemAvailability updates a single item availability on Deliveroo.
// It uses the default menu ID generated by the menu mapper ("main"). locationID
// vide cible le compte "principal" du marchand (comportement historique,
// identique pour un marchand mono-compte) ; locationID renseigné cible ce
// compte précis.
func (s *DeliverooService) ToggleItemAvailability(ctx context.Context, merchantID, locationID, itemID string, available bool) error {
	account, err := s.repo.resolveAccount(ctx, merchantID, locationID)
	if err != nil {
		return err
	}
	...
	return s.client.UpdateUnavailabilities(ctx, account.BrandID, defaultDeliverooMenuID, account.LocationID, payload)
}
...
func (s *DeliverooService) UpdatePreparationTime(ctx context.Context, merchantID, locationID string, preparationTimeMinutes int) error {
	...
	if account.BrandID == "" {
		...
		brandID, syncErr := s.ValidateAndSyncBrandID(ctx, merchantID, account.LocationID)
		if syncErr != nil {
			return syncErr
		}
	...
	return s.client.UpdateSiteWorkloadMode(ctx, account.BrandID, account.LocationID, mode)
}
...
// CloseStoreTemporary temporarily closes the Deliveroo site for a fixed
// duration. locationID vide cible le compte "principal" du marchand
// (comportement historique, identique pour un marchand mono-compte) ;
// locationID renseigné cible ce compte précis.
func (s *DeliverooService) CloseStoreTemporary(ctx context.Context, merchantID, locationID string, durationMinutes int) error {
	account, err := s.repo.resolveAccount(ctx, merchantID, locationID)
	if err != nil {
		return err
	}
	...
	return s.client.CloseSiteTemporary(ctx, account.LocationID, offlineUntil)
}

// ValidateAndSyncBrandID exécute le scénario de validation du Brand ID pour un
// compte Deliveroo donné (siteID) : appelle Deliveroo, et met à jour le
// brand_id de ce compte précis. siteID vide résout le compte "principal" du
// marchand (comportement historique, identique pour un marchand mono-compte).
func (s *DeliverooService) ValidateAndSyncBrandID(ctx context.Context, merchantID, siteID string) (string, error) {
	if siteID == "" {
		var err error
	...
	err = s.repo.UpdateMerchantBrandID(ctx, merchantID, siteID, brandID)
	if err != nil {
