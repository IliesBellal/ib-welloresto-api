...
import (
	...
	"strings"
	"time"
	"welloresto-api/internal/client"
...
type DeliverooService struct {
	repo   *repositories.DeliverooRepository
	db     *sql.DB
	log    *zap.Logger
	client *client.DeliverooClient
}

func NewDeliverooService(repo *repositories.DeliverooRepository, db *sql.DB, log *zap.Logger) *DeliverooService {
	dc := client.NewDeliverooClient()
