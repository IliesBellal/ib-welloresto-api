...
type UberEatsService struct {
	repo       *repositories.UberEatsRepository
	db         *sql.DB
	log        *zap.Logger
	httpClient *client.UberEatsClient
}

func NewUberEatsService(repo *repositories.UberEatsRepository, db *sql.DB, log *zap.Logger) *UberEatsService {
	uec := client.NewUberEatsClient()
